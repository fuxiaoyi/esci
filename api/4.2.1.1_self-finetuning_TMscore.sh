...
# load python module 
your_local_foldseek_path="/home/bingxing2/ailab/scxlab0094/hantao/project/internTA/conda_env/proteinRL_cp310/bin"
...
folder_path="/home/bingxing2/ailab/scxlab0094/hantao/project/internTA/proteinGflownet/ProtRL/experiments/Self-finetuning/s-FT_TM-score" # in which you want the results to be in 
...
do
    ...
    if [ $i != 0 ]; then
    echo Train started
    python "${folder_path}/self_train_TMscore.py" --iteration_num "${i}" --label "${label}" --cwd "${folder_path}"
    else
    ...
    python "${folder_path}/seq_gen.py" --iteration_num "${i}" --label "${label}" --cwd "${folder_path}"
    
    # Fold the sequences with ESM fold
    echo Folding started
    python "${folder_path}/esmfold.py" --iteration_num "${i}" --label "${label}" --cwd "${folder_path}"
