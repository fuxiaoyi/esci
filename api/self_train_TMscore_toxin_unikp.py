...
device_name = "cpu"  # Force CPU usage
...
def _load_kcat_map(kcat_csv: str, col_use: str = "pred_linear"):
    """读取 UniKP kcat 文件，返回 {name: float}。支持 id, pred_linear, pred_log10 列。"""
    if kcat_csv is None or not os.path.exists(kcat_csv):
        return {}
    try:
        df = pd.read_csv(kcat_csv, sep=None, engine="python")
    except Exception:
        df = pd.read_csv(kcat_csv, sep="\t")
    cols = {c.lower(): c for c in df.columns}
    idc = cols.get("id") or cols.get("name") or list(df.columns)[0]
    kc = cols.get(col_use.lower())
    if kc is None:
        # 兜底：如果用户传的是 pred_log10 但文件没有，就试试 pred_linear
...
def _minmax_norm(val: float, vmin: float, vmax: float) -> float:
    if vmax <= vmin:
        return 1.0  # 全相等时不做区分，给 1
...
def generate_dataset(
    ...
    TMscores: str,
    tox_csv: str = None,
    tox_mode: str = "penalize",    # "penalize" 或 "filter"
    ...
    # kcat 相关
    kcat_csv: str = None,
    kcat_col: str = "pred_linear", # "pred_linear" 或 "pred_log10"
    kcat_beta: float = 1.0,        # kcat 因子幂次
    kcat_drop_if_missing: bool = False,  # 没有 kcat 的样本是否丢弃
):
    """融合 FASTA + Foldseek TM + ToxinPred2 + UniKP kcat -> 训练样本并计算 weight。"""
    ...
    ml_values = [float(v["ml"]) for v in tox_map.values()]
    #ml_median = np.median(ml_values)
    ml_median = np.quantile(ml_values, 0.70)

    # 3) 读 kcat
    kcat_map = _load_kcat_map(kcat_csv, kcat_col)
    # 只统计当前批次里出现的序列的 kcat 用于归一化
    k_vals = [v for k, v in kcat_map.items() if k in sequences_rep]
    kmin = min(k_vals) if k_vals else 0.0
    kmax = max(k_vals) if k_vals else 1.0

    # 4) 读 Foldseek TM 输出（query,target,alntmscore,qtmscore,ttmscore,alnlen）
    ...
    for entry in lines:
        ...
        try:
            ...
            TM_norm_que = float(parts[4]) # 这里用 ttmscore；如用 qtmscore 则改成 parts[3]
        ...
        # kcat 因子
        if name in kcat_map:
            kcat_norm = _minmax_norm(kcat_map[name], kmin, kmax)
            kcat_factor = (kcat_norm ** float(kcat_beta))
        else:
            if kcat_drop_if_missing:
                continue
            kcat_factor = 1.0
...
if __name__ == '__main__':
    ...
    parser.add_argument("--kcat_csv", type=str, default=None,
                        help="UniKP kcat CSV/TSV，需包含 id 及 pred_linear / pred_log10 列")
    parser.add_argument("--kcat_col", type=str, default="pred_linear",
                        choices=["pred_linear", "pred_log10"], help="使用哪一列作为 kcat 分数")
    parser.add_argument("--kcat_beta", type=float, default=1.0,
                        help="kcat 因子幂次（越大越偏向高 kcat）")
    parser.add_argument("--kcat_drop_if_missing", action="store_true",
                        help="若样本在 kcat 文件中缺失则丢弃")
    ...
    kcat_csv = args.kcat_csv or f'{cwd}kcat/{label}_kcat_iteration{iteration_num-1}.csv'

    # 构造训练数据（融合 tox 与 kcat）
    data = generate_dataset(
        ...
        kcat_csv=kcat_csv,
        kcat_col=args.kcat_col,
        kcat_beta=args.kcat_beta,
        kcat_drop_if_missing=bool(args.kcat_drop_if_missing),
    )
    ...
    best_sequences = df.iloc[:201, :]['sequence'].to_list()  # 你之前就是 51，这里保持
    ...
    for seq in best_sequences:
        ...
        available_space = 1021 - control_code_len
    ...
    raw_datasets["train"] = load_dataset(extension, data_files=data_files, split=f"train[10%:]", cache_dir='.')
    raw_datasets["validation"] = load_dataset(extension, data_files=data_files, split=f"train[:10%]", cache_dir='.')
