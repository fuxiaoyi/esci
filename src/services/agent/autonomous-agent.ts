...
import {DefaultAgentRunModel} from "./agent-run-model";
...
class AutonomousAgent {
  ...
  addTasksIfWorklogEmpty = () => {
    if (this.workLog.length > 0 || !this.model) return;
  ...
  async chat(message: string) {
    ...
    const newGoal = chatWork.getResult() || "";
    this.api.updateAgentGoal(newGoal);
    this.workLog = [new StartGoalWork(this)];
    this.model = new DefaultAgentRunModel(newGoal.trim());
    this.run().then(console.log).catch(console.error);
  }
