...
export const get = async <T extends z.ZodTypeAny>(
  ...
  // Special case for SID endpoints - use Next.js API routes instead of external backend
  const isSidEndpoint = path.startsWith('/auth/sid/');
  const baseUrl = isSidEndpoint ? env.NEXT_PUBLIC_VERCEL_URL : env.NEXT_PUBLIC_BACKEND_URL;
  const apiPath = isSidEndpoint ? '/api' : env.NEXT_PUBLIC_BACKEND_PATH;
