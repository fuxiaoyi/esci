...
const ChatMessage = ({ message }: { message: Message }) => {
  ...
  if (message.type === MESSAGE_TYPE_GOAL && !isAction(message)) {
    return <div className="pb-2 text-2xl sm:text-4xl text-slate-5">{message.value}</div>;
  }
  return (
    <div
      className={clsx(
      ...
      {isAction(message) ? (
        <>
          ...
          <div>
            ...
            {!message.info?.toLowerCase().includes('.mp3') && <SourceCard content={message.info || ""} />}
          </div>
        </>
      ) : (
        <>
...
const FAQ = () => {
  return (
    <p>
      ...
      <a href="https://docs.kongfoo.ai/essentials/FAQ" className="text-sky-500">
        FAQ
      </a>
    </p>
  );
