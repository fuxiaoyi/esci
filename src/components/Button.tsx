...
const Button = forwardRef((props: ButtonProps, ref: ForwardedRef<HTMLButtonElement>) => {
  ...
  return (
    <button
      ...
      className={clsx(
        "relative rounded-lg border-2 border-slate-0 px-4 py-1 font-bold text-white transition-all duration-300 sm:px-10 sm:py-3",
        props.disabled && "cursor-not-allowed border-amber-600 bg-amber-600 text-white",
        props.disabled ||
          // "cursor-pointer bg-[#1E88E5]/80 text-white hover:bg-[#0084f7] hover:shadow-lg",
          "cursor-pointer bg-amber-600 text-white hover:bg-amber-600 hover:shadow-lg",
